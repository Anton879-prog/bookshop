...
public class CacheManager {
    ...
    private static final int MAX_CACHE_SIZE = 10_000; // Максимальный размер кэша
    ...
    public void clearPublisherCache(String publisherName) {
        ...
        cache.remove(key);
        LOGGER.info("🗑 Кэш очищен для издателя: {}", publisherName);
    }

    public void clearBookCache(Long bookId) {
        String key = "book_" + bookId;
        cache.remove(key);
        LOGGER.info("🗑 Кэш очищен для книги с ID: {}", bookId);
    }

    public void clearAuthorCache(Long authorId) {
        String key = "author_" + authorId;
        cache.remove(key);
        LOGGER.info("🗑 Кэш очищен для автора с ID: {}", authorId);
    }

    public void clearByPrefix(String prefix) {
        cache.keySet().removeIf(key -> key.startsWith(prefix));
        LOGGER.info("🗑 Очищен кэш для префикса: {}", prefix);
    }
    ...
    private void removeOldestEntry() {
        ...
        oldestEntry.ifPresent(entry -> {
            ...
            LOGGER.info("🗑 Удалён самый старый кэш-ключ: {}", entry.getKey());
        });
    }
    ...
    public void startCacheCleanupTask() {
        try {
            ...
            cache.entrySet().removeIf(entry -> {
                ...
                if (expired) {
                    LOGGER.info("🗑 Удаляется ключ: {}", entry.getKey());
                }
                return expired;
            });
